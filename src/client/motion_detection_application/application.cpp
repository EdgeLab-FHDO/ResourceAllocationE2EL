...
/*
		...
		if(connection_TCP->receive_data(data, data_size) < 0)
		{
			cout << connection_TCP->get_last_error() << endl;
			return;
		}
		...
		cout << "number of contours is " << (int)data[0] << endl;
	}
}
...
void Application::initialize(ClientTCP * connection_TCP, int dummy_data_size)
{
	if(!initialized)
	{
		...
		// TODO remove the use of dummy_data_size
		this->data_size = dummy_data_size;
